...
public class OpaAuthorizer implements Authorizer {
    ...
    private boolean allow(Msg.Input input) {
        try {
            ...
            boolean result = (boolean) gson.fromJson(br.readLine(), Map.class).get("result");
            log.info("user: {}, response code: {}, result: {}, data: {}",
                    input.getSession().sanitizedUser(), conn.getResponseCode(), result, data
            );
            return result;
        } catch (IOException e) {
            return allowOnError;
    ...
    public void configure(Map<String, ?> configs) {
        ...
        allowOnError = Boolean.valueOf(getValueOrDefault(OPA_AUTHORIZER_DENY_ON_ERROR_CONFIG, "false"));
        initialCapacity = Integer.parseInt(getValueOrDefault(OPA_AUTHORIZER_CACHE_INITIAL_CAPACITY_CONFIG, "100"));
        maximumSize = Integer.parseInt(getValueOrDefault(OPA_AUTHORIZER_CACHE_MAXIMUM_SIZE_CONFIG, "100"));
        expireAfterMs = Long.parseLong(getValueOrDefault(OPA_AUTHORIZER_CACHE_EXPIRE_AFTER_MS_CONFIG, "600000"));
